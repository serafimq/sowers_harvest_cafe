...
function SelectTrigger({ className = 'w-full', children, ...props }: SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger
      data-slot="select-trigger"
      className={cn(
        "flex h-[62px] w-full items-center justify-between border border-white/10 bg-white/5 px-6 py-2 text-base ring-offset-white text-white placeholder:text-white/80 focus-visible:ring-[3px] focus:ring-orange disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
